...
function twoSum(numbers, target) {
  for (let i = 0; i < numbers.length; i += 1) {
    let firstValue = numbers[i]; //1
    let secondValue = target - firstValue; //3
    if (numbers.includes(secondValue) && numbers.indexOf(secondValue) !== i) {
      return [i, numbers.indexOf(secondValue)];
    }
  }
  return [];
}
...
console.log(twoSum([1234, 5678, 9012], 14690)); //returns [1,2]
